...
export class ProjectsService{
 ...
 async get(id:string,user:AuthUser){await this.requireAccess(id,user);return this.db.project.findUnique({where:{id},include:{members:{include:{user:{select:{id:true,name:true,email:true,role:true}}}}}})}
 async update(id:string,user:AuthUser,body:unknown){await this.requireAdmin(id,user);return this.db.project.update({where:{id},data:projectDto.partial().parse(body)})}
 async remove(id:string,user:AuthUser){await this.requireAdmin(id,user);await this.db.project.delete({where:{id}})}
 async addMember(id:string,user:AuthUser,userId:string){await this.requireAdmin(id,user);if(!await this.db.user.findUnique({where:{id:userId}}))throw new NotFoundException("User not found");return this.db.projectMember.upsert({where:{projectId_userId:{projectId:id,userId}},create:{projectId:id,userId},update:{}})}
 async removeMember(id:string,user:AuthUser,userId:string){const p=await this.requireAdmin(id,user);if(p.creatorId===userId)throw new UnprocessableEntityException("Project creator cannot be removed");await this.db.projectMember.deleteMany({where:{projectId:id,userId}})}
